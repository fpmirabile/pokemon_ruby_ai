...
class BHServer:
    ...
    def __init__(
        ...
        # Actions taken during the current episode (number of UPDATEs called)
        self.actions = 0
        # Emulator status
        # Did the client just call START? Access ONLY by client_started()
        ...
        # Path to the current save file (from parent of BizHawk dir). Updated with load_save()
        self.save = ""
        # Controls dict to be passed to emulator, or received from emulator if recording
        self.controls = {}
        # Client-Settable
        # Stores {VAR: (DATATYPE, VAL)}. Utilized by SET and GET statements from clients
        self.data = dict()
    ...
    def run(self):
        ...
        while True:
            ...
            self.log('Accepted connection from {}:{}'.format(
                address[0], address[1]))
    ...
    def handle_msg(self, msg, client_socket):
        ...
        # Indicates whether msg's value has changed (i.e. any POST with payload)
        new_msg = True
        ...
        while new_msg:
            ...
            for stmt in msg.split("; "):
                ...
                #
                    ...
                    # Dictionaries
                    if var.startswith("screenshots "):
                        response += self.screenshots[int(var[11:])
                                                     ].decode("utf-8")
                    elif var == "controls":
                    ...
                    # Strings
                        ...
                        response += self.save
                    # Integers
                    elif var == "update_interval":
                        response += str(self.update_interval)
                    elif var == "speed":
                        response += str(self.speed)
                    elif var == "frameskip":
                        response += str(self.frameskip)
                    # Booleans
                    elif var == "exit":
                        ...
                        response += str(self.sound)
                    elif var == "guessed":
                        response += str(self.guessed)
                    ...
                    #
                        ...
                        # Are we getting the value of a variable?
                            ...
                            if val is None:
                                ...
                                if val[0] == "DICT":
                                    response += dict_as_str(val[1])
                                else:
                                    response += val[0] + " " + str(val[1])
                    ...
                    #
                        ...
                        if match.group(2)[0].isdigit():
                            ...
                            if existing_var is None:
                                print(
                                    "ERROR: Attempt to index non-existent list " + var)
                                return
                        ...
                        if data_type.endswith("[]"):
                            if not isinstance(val, list) and not has_idx:
                                print(
                                    "ERROR: List initialization value does not match datatype")
                                return
                            ...
                            # Are we setting a list element or re-initializing the list?
                                ...
                                if not has_idx:
                                    ...
                                    # Throw an error
                                        ...
                                        print(
                                            "ERROR: List " + var + "[" + str(idx) + "]" + " index out of range")
                                        return
                ...
                #
                    ...
                    # BizHawk expects connection to close after each Lua method call
                    ...
                    if screenshot_idx != -1:
                        ...
                        # Using unquote because urlsafe_ doesn't work
                        img = base64.b64decode(unquote_plus(screenshot))
        ...
        if response.endswith("; "):
            client_socket.send(response.encode("utf-8")[:-2])
        else:
            client_socket.send(response.encode("utf-8"))
